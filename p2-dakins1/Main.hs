...
getDict 0 = fmap buildDict longDictIO
getDict 1 = return $ buildDict standardDict
getDict 2 = return $ buildDict shortDict
getDict 3 = return $ buildDict tinyDict
getDict _ = return $ buildDict tenWords 
...
main = do
  ...
  case (optHelp opts, optSolve opts, optTest opts) of
    (True, _, _) -> helpIO
    (_, _, True) -> runTests (optQuiet opts)
    (_, k, _) -> do 
